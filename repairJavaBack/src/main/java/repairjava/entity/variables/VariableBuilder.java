package repairjava.entity.variables;
...
public class VariableBuilder extends VoidVisitorAdapter<Void>{
    ...
    public void visit(Parameter parameter, Void arg){
        Variable var = new Variable(parameter.getName().getIdentifier(),parameter.getType(), Variable.SpecialType.PARAM,Variable.getScope(parameter), parameter.getRange());
        insertVI(var);
        paramList.add(var);
        paramSize++;
    ...
    public void visit(ReturnStmt returnStmt,Void arg){
        if(returnStmt.getExpression().isPresent()){
            ...
            Variable var=new Variable("r"+return_Index,methodDeclaration.getType(), Variable.SpecialType.RETURN,Variable.getScope(returnStmt), returnStmt.getRange());
            return_Index++;
            returnList.add(var);
            insertVI(var);
    ...
    public void visit(VariableDeclarator variableDeclarator, Void arg) {
        Variable var = new Variable(variableDeclarator.getName().getIdentifier(),
                variableDeclarator.getType(), Variable.SpecialType.LOCAL,Variable.getScope(variableDeclarator), variableDeclarator.getRange());
        insertVI(var);
        V2V.put(var,variableDeclarator);
    ...
    public void getVariablesFromNode(Node node,Set<Variable>variableList){
        List<SimpleName>simpleNames= RepairUtils.getVariableNameFromNode(node);
