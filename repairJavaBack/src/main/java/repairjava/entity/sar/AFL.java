...
import repairjava.entity.program.BlockBuilder;
...
import repairjava.entity.variables.*;
...
public class AFL {
        ...
        for (int i = 0; i < buggyBlockChildren.size();) {
            ...
            if(result.isFault){
                ...
                if(buggyBlockNodesValueIndexes.get(i).outValueIndex != -1) {
                    ...
                    endIndex = buggyStackBuffer.endIndex;
                }
                ...
                if (correctBlockNodesValueIndexes.get(i).outValueIndex!=-1) {
                    ...
                    endIndex = correctStackBuffer.endIndex;
                }
    ...
    private ArrayList<Integer> getLoopIndexes(BlockNode blockNode, StackBuffer stackBuffer){
//        ArrayList<Integer> loopIns = new ArrayList<>();
        if(stackBuffer.getOriginalIndex(blockNode.getChildBlocks().get(0).getLineNumbers().get(0))!=-1){
            return stackBuffer.getAllIndexes(blockNode.getChildBlocks().get(0).getLineNumbers().get(0));
        }
...
//                    faultBlock = whileCond;
    ...
    static class StackBuffer{
        int beginIndex;
        int endIndex;
        int outIndex;
        ArrayList<String> breakPointLineIndexes;
...
//                result.setNeedRecall(false);
    ...
    private boolean getCondFlag(BlockNode condBlock, BlockNode bodyNode, StackBuffer stackBuffer){
    ...
    private boolean getBodyFlag(BlockNode bodyNode, StackBuffer stackBuffer){
        int i = 0;
...
//                    faultBlock = ifBody;
    ...
    public int getEmptyBlockIndex(BlockNode blockNode, StackBuffer stackBuffer){
    ...
    public int getJumpBlockIndex(JumpBlock jumpBlock, StackBuffer stackBuffer) {
        int index;
        index = stackBuffer.getOriginalIndex(jumpBlock.getLineNumber());
        return index;
    }

    private ArrayList<ValueIndex> getChildrenIndexes(ArrayList<BlockNode> children, StackBuffer stackBuffer){
...
//    private ArrayList<ValueIndex> getBodyIndexes(MethodBuilder methodBuilder){
//        ArrayList<BlockNode> children;
//        children = methodBuilder.getBlockBuilder().getBlockNodes();
//        StackBuffer stackBuffer = new StackBuffer(0 , methodBuilder.getBreakPointLines().size(), methodBuilder.getBreakPointLines());
//        return getChildrenIndexes(children, stackBuffer);
//    }

    private int getBlockValueBeginIndex(BlockNode blockNode, StackBuffer stackBuffer) {
        int index = -1;
