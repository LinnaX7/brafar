...
import repairjava.entity.sar.repair.RepairInsert;
...
public class RepairUtils {
    ...
    public static Pair<Integer,Integer> getBlockRange(MethodBuilder methodBuilder,Integer metaBlockIndex) {
        ...
        for(Node child:blockNode.getTreeNodes()){
            ...
            int index=getIndex(((BlockStmt) parent),(Statement) child);
            if(index!=-1)
                order.add(index);
...
//        return new Pair<>(first, second);
    ...
    public static boolean isDefBeforeUse(Variable b,int useIndex,Node useNode,Common common){
        if(b.getDefine()<common.blockIndex.peek()){
            ...
            if(buggy.getParentNode().get().getParentNode().get() instanceof  ExpressionStmt){
    ...
    public static void checkDef(Variable b,Variable c,int useIndex,Node before,Common common,Node wholeNode){
        if(!isDefBeforeUse(b,useIndex,wholeNode,common)){
            ...
            if(buggyDecl.getVariables().size()!=1){
                ...
                if(buggy.getInitializer().isPresent()) {
                    AssignExpr assignExpr=new AssignExpr(new NameExpr(buggy.getName()),
                            buggy.getInitializer().get(), AssignExpr.Operator.ASSIGN);
            ...
            if(c!=null){
                if(common.variableMatch.getC2bMatch().containsKey(c)){
                    ...
                    if(RepairUtils.isDef(buggy)){
                        List<VariableDeclarator>decls=RepairUtils.getVariableDecl(buggy);
                    ...
                    if(flag){
                        ...
                        if(f)
                            RepairUtils.checkDef(common.variableMatch.getC2bMatch().get(c),c,index,simpleName.getParentNode().get(),common,buggy);
                    }
                }else{
                    ...
                    RepairInsert insertStmt=new RepairInsert(common);
                    insertStmt.repairInsert(common.buggyMethodBuilder.getMetaBlockNodes().get(block).getParentNode(),expressionStmt,correctDecl);
                    simpleName.setIdentifier(common.variableMatch.getC2bMatch().get(c).getName());
