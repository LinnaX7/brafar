package repairjava.entity.monitor;


import repairjava.entity.program.EnvironmentConfig;
...
public class JDIDebuggerExecutor {
    ...
    public final static String SINGLE_JUNIT_PATH = Paths.get(ROOT_PATH, "src", "main", "java", "repairjava", "entity", "monitor", "SingleJUnitTestRunner.java").toString();
    public final static String CLASS_PATH = Paths.get(ROOT_PATH, "src", "main", "java").toString();
    public final static String JDI_DEBUGGER_PATH = Paths.get(ROOT_PATH, "src", "main", "java", "repairjava", "entity", "monitor", "JDIDebugger.java").toString();

    public static void compileTest(ProgramBuilder pb, TesterBuilder tb) throws IOException {
        String compileTesteeCMD = String.format("cd %s && javac -g -cp %s;. %s", pb.getParentPath(),
                TOOL_PATH, pb.getFilePath());

        String compileTesterCMD = String.format("cd %s && javac -g -cp %s;%s;%s;. %s", tb.getJavaParentPath(),
                TOOL_PATH, JUNIT_PATH, pb.getClassPath()+ EnvironmentConfig.SLASH, tb.getFilePath());
        ...
        try {
            ...
            if (process != null) {
                InputStreamReader read = new InputStreamReader(process.getErrorStream(), "GBK");
                ...
                OutputStreamWriter write = new OutputStreamWriter(new FileOutputStream(Paths.get("tmp","compile.log").toString()), "GBK");
...
//        }
    ...
    public static void executeAllTest(ProgramBuilder pb, TesterBuilder tb, String testeeMethod) throws IOException {
        String cdJDIDebuggerParent = String.format("cd %s", CLASS_PATH);
        String executeCMD = String.format("java -cp %s;%s;. %s %s %s %s %s %s %s", TOOL_PATH, pb.getClassPath()+EnvironmentConfig.SLASH, JDIDebugger.JDI_DEBUGGER,
                "Mode0", pb.getClassPath()+EnvironmentConfig.SLASH, pb.getClassName(), testeeMethod, tb.getClassPath()+EnvironmentConfig.SLASH, tb.getClassName());
    ...
    public static void executeTest(ProgramBuilder pb, TesterBuilder tb, String testeeMethod, String testerMethod) throws IOException {
        String cdJDIDebuggerParent = String.format("cd %s", CLASS_PATH);
        String executeCMD = String.format("java -cp %s;%s;. %s %s %s %s %s %s %s %s", TOOL_PATH, pb.getClassPath()+EnvironmentConfig.SLASH, JDIDebugger.JDI_DEBUGGER,
                "Mode1", pb.getClassPath()+EnvironmentConfig.SLASH, pb.getClassName(), testeeMethod, tb.getClassPath()+EnvironmentConfig.SLASH, tb.getClassName(), testerMethod);
        executeCmd(cdJDIDebuggerParent, executeCMD);
    }
